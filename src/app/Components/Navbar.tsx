...
const Navbar = ({ refArr }: { refArr: RefObject<HTMLDivElement | null>[] }) => {
    ...
    return (
        <header className="w-full p-5 bg-[url(/images/noise.png)] navDiv z-20">
            <nav className="customContainer flex-center flex-col md:flex-row gap-6 sm:gap-8 md:gap-0
                md:justify-between">
                <Link href='#home' className="flex-center gap-2 font-semibold cursor-pointer" onClick={handleClickToHome}>
                ...
                <ul className="flex-center gap-6 sm:gap-12">
                    {
                        navItems.map((item) => (
                            <Link href={`#${item.title}`} key={item.id} className="cursor-pointer"
                                onClick={() => {
                                    if (refArr[item.id].current) {

                                        gsap.to(window, {
                                            duration: 1,
                                            ease: 'power1.inOut',
                                            scrollTo: {
                                                y: refArr[item.id].current!,
                                                offsetY: 50,
                                            }
                                        })
                                    }
